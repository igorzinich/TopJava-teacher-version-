...
#### Apply 11_14_vds_restrict_modification.patch
 ...
 - В `UserService` добавил защиту от изменения `Admin/User` для профиля `VDS` (в `UserService` заинжектил `Environment` и сделал проверку на наличие профиля `HEROKU`)
...
Файл пропертей с паролями не принято держать в репозитории (даже если он _private_).  
...
### **Опционально: зарегистрировать хостинг и задеплоить в него свое приложение** 
